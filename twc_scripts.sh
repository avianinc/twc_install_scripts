...
# https://drive.google.com/file/d/1Dhmes9mSQqqphrocZt8rOLp9DSwL2pLL/view?usp=sharing
# 1Dhmes9mSQqqphrocZt8rOLp9DSwL2pLL
# install_flex_centos7.sh
echo
echo "Get flex_centos7.sh installation scrip - not avalible from the download site!!!!"
wget --load-cookies /tmp/cookies.txt "https://docs.google.com/uc?export=download&confirm=$(wget --quiet --save-cookies /tmp/cookies.txt --keep-session-cookies --no-check-certificate 'https://docs.google.com/uc?export=download&id=1Dhmes9mSQqqphrocZt8rOLp9DSwL2pLL' -O- | sed -rn 's/.*confirm=([0-9A-Za-z_]+).*/\1\n/p')&id=1Dhmes9mSQqqphrocZt8rOLp9DSwL2pLL" -O install_flex_centos7.sh && rm -rf /tmp/cookies.txt
echo
